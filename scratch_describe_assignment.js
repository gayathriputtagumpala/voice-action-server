...
async function run() {
  ...
  const url = `${baseUrl}/hcmRestApi/resources/11.13.18.05/workers/${encodedPersonId}/child/workRelationships/${WorkRelationshipId}/child/assignments/${encodedAssignmentId}/describe`;
  ...
  try {
    ...
    const outFile = path.join(scratchDir, 'describe_assignment.json');
    fs.writeFileSync(outFile, JSON.stringify(res.data, null, 2));
    console.log('Saved describe assignment metadata to:', outFile);
  } catch (err) {
