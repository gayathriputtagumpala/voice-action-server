const axios = require('axios');
const https = require('https');

async function test() {
  const workerUrl = 'https://fa-eubg-test-saasfademo1.ds-fa.oraclepdemos.com/hcmRestApi/resources/11.13.18.';
  
  const agent = new https.Agent({ rejectUnauthorized: false });
  const auth = 'Basic dXNlcl9yMTRfYTJmOnFvMkgqNlcj';
  
  const body = { 
    "ActionCode": "ASG_CHANGE", 
    "DepartmentId": 300000047597383 // Consulting West UK (Validated for this BU in 1405 test)
  };

  console.log(`--- Testing Change US Worker to UK Department (Known Valid for BU) ---`);
  try {
    const res = await axios.patch(workerUrl, body, {
      httpsAgent: agent,
      headers: {
        'Content-Type': 'application/json',
        'Authorization': auth,
        'Effective-Of': 'RangeMode=UPDATE;RangeStartDate=2026-05-09'
      }
    });
    console.log(`SUCCESS: ${res.status}`);
  } catch (err) {
    console.log(`FAILED: ${err.response?.status}`);
    console.log(`ERROR: ${JSON.stringify(err.response?.data)}`);
  }
}

test();
