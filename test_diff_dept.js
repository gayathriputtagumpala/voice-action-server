const axios = require('axios');
const https = require('https');

async function test() {
  const workerUrl = 'https://fa-eubg-test-saasfademo1.ds-fa.oraclepdemos.com/hcmRestApi/resources/11.13.18.';
  
  const agent = new https.Agent({ rejectUnauthorized: false });
  const auth = 'Basic dXNlcl9yMTRfYTJmOnFvMkgqNlcj';
  
  const variations = [
    { name: 'Change to West UK (Name only)', body: { "ActionCode": "ASG_CHANGE", "DepartmentName": "Consulting West UK" } },
    { name: 'Change to West UK (ID only)', body: { "ActionCode": "ASG_CHANGE", "DepartmentId": 300000047597383 } }
  ];

  for (const v of variations) {
    console.log(`--- Testing ${v.name} ---`);
    try {
      const res = await axios.patch(workerUrl, v.body, {
        httpsAgent: agent,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': auth,
          'Effective-Of': 'RangeMode=UPDATE;RangeStartDate=2025-05-01'
        }
      });
      console.log(`SUCCESS: ${res.status}`);
    } catch (err) {
      console.log(`FAILED: ${err.response?.status}`);
      console.log(`ERROR: ${JSON.stringify(err.response?.data)}`);
    }
  }
}

test();
